CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_VALPROATE_PSYCHIATRY (
    PERSON_ID VARCHAR COMMENT 'Unique identifier for the person',
    SK_PATIENT_ID VARCHAR COMMENT 'Surrogate key for the patient',
    HAS_PSYCH_EVENT BOOLEAN COMMENT 'Always TRUE for persons in this table, indicating at least one psychiatry-related event meeting criteria',
    EARLIEST_PSYCH_EVENT_DATE DATE COMMENT 'Earliest date of any psychiatry-related event for the person',
    LATEST_PSYCH_EVENT_DATE DATE COMMENT 'Latest date of any psychiatry-related event for the person',
    ALL_PSYCH_OBSERVATION_IDS ARRAY COMMENT 'Array of unique observation IDs (from OBSERVATION table) related to psychiatry events',
    ALL_PSYCH_CONCEPT_CODES ARRAY COMMENT 'Array of all distinct psychiatry-related medical concept codes found for the person',
    ALL_PSYCH_CONCEPT_DISPLAYS ARRAY COMMENT 'Array of display terms for the psychiatry concept codes (from MAPPED_CONCEPTS.CODE_DESCRIPTION)',
    ALL_PSYCH_CODE_CATEGORIES_APPLIED ARRAY COMMENT 'Array of code categories applied (will contain \'-PSYCH\'-)'
)
COMMENT = 'Dimension table aggregating psychiatry-related events for each person, using MAPPED_CONCEPTS and OBSERVATION table. It identifies individuals with any psychiatry event meeting criteria defined in VALPROATE_PROG_CODES (category PSYCH).'
TARGET_LAG = '4 hours'
REFRESH_MODE = AUTO
INITIALIZE = ON_CREATE
WAREHOUSE = NCL_ANALYTICS_XS
AS
WITH BasePsychObservations AS (
    -- Selects psychiatry-related observations.
    -- LOOKBACK_YEARS_OFFSET is NULL for all PSYCH codes, so all historical records are considered.
    SELECT
        PP."person_id" AS PERSON_ID,
        PAT."sk_patient_id" AS SK_PATIENT_ID,
        O."id" AS OBSERVATION_ID,
        O."clinical_effective_date"::DATE AS PSYCH_EVENT_DATE,
        MC.CONCEPT_CODE AS PSYCH_CONCEPT_CODE,
        MC.CODE_DESCRIPTION AS PSYCH_CONCEPT_DISPLAY,
        VPC.CODE_CATEGORY AS PSYCH_CODE_CATEGORY -- This will be 'PSYCH'
    FROM
        "Data_Store_OLIDS_Dummy"."OLIDS_MASKED"."OBSERVATION" AS O
    JOIN
        DATA_LAB_OLIDS_UAT.REFERENCE.MAPPED_CONCEPTS AS MC
        ON O."observation_core_concept_id" = MC.SOURCE_CODE_ID
    JOIN
        DATA_LAB_OLIDS_UAT.REFERENCE.VALPROATE_PROG_CODES AS VPC
        ON MC.CONCEPT_CODE = VPC.CODE
    JOIN
        "Data_Store_OLIDS_Dummy"."OLIDS_MASKED"."PATIENT_PERSON" AS PP
        ON O."patient_id" = PP."patient_id"
    JOIN
        "Data_Store_OLIDS_Dummy"."OLIDS_MASKED"."PATIENT" AS PAT
        ON PP."patient_id" = PAT."id"
    WHERE
        VPC.CODE_CATEGORY = 'PSYCH'
        -- The following condition is effectively TRUE for all PSYCH codes as their LOOKBACK_YEARS_OFFSET is NULL.
        -- It's included for structural consistency and if offsets were to be introduced for PSYCH later.
        AND (
            VPC.LOOKBACK_YEARS_OFFSET IS NULL OR
            O."clinical_effective_date"::DATE >= DATEADD(YEAR, VPC.LOOKBACK_YEARS_OFFSET, CURRENT_DATE())
        )
),
PersonLevelPsychAggregation AS (
    -- Aggregates psychiatry information for each person.
    SELECT
        PERSON_ID,
        ANY_VALUE(SK_PATIENT_ID) as SK_PATIENT_ID,
        MIN(PSYCH_EVENT_DATE) AS EARLIEST_PSYCH_EVENT_DATE,
        MAX(PSYCH_EVENT_DATE) AS LATEST_PSYCH_EVENT_DATE,
        ARRAY_AGG(DISTINCT OBSERVATION_ID) WITHIN GROUP (ORDER BY OBSERVATION_ID) AS ALL_PSYCH_OBSERVATION_IDS,
        ARRAY_AGG(DISTINCT PSYCH_CONCEPT_CODE) WITHIN GROUP (ORDER BY PSYCH_CONCEPT_CODE) AS ALL_PSYCH_CONCEPT_CODES,
        ARRAY_AGG(DISTINCT PSYCH_CONCEPT_DISPLAY) WITHIN GROUP (ORDER BY PSYCH_CONCEPT_DISPLAY) AS ALL_PSYCH_CONCEPT_DISPLAYS,
        ARRAY_AGG(DISTINCT PSYCH_CODE_CATEGORY) WITHIN GROUP (ORDER BY PSYCH_CODE_CATEGORY) AS ALL_PSYCH_CODE_CATEGORIES_APPLIED
    FROM BasePsychObservations
    GROUP BY PERSON_ID
)
SELECT
    pla.PERSON_ID,
    pla.SK_PATIENT_ID,
    TRUE AS HAS_PSYCH_EVENT,
    pla.EARLIEST_PSYCH_EVENT_DATE,
    pla.LATEST_PSYCH_EVENT_DATE,
    pla.ALL_PSYCH_OBSERVATION_IDS,
    pla.ALL_PSYCH_CONCEPT_CODES,
    pla.ALL_PSYCH_CONCEPT_DISPLAYS,
    pla.ALL_PSYCH_CODE_CATEGORIES_APPLIED
FROM PersonLevelPsychAggregation pla;
