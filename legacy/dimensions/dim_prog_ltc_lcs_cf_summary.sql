CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_SUMMARY (
...
WITH BasePopulation AS (
    ...
    SELECT DISTINCT
        ...
        bp.SK_PATIENT_ID,
        age.AGE
    FROM DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_LTC_LCS_CF_BASE_POPULATION bp
...
-- AF indicators
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_AF_61 af61
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_AF_62 af62
    USING (PERSON_ID)
-- CKD indicators
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_CKD_61 ckd61
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_CKD_62 ckd62
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_CKD_63 ckd63
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_CKD_64 ckd64
    USING (PERSON_ID)
-- CVD indicators
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_CVD_61 cvd61
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_CVD_62 cvd62
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_CVD_63 cvd63
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_CVD_64 cvd64
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_CVD_65 cvd65
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_CVD_66 cvd66
    USING (PERSON_ID)
-- CYP Asthma indicator
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_CYP_AST_61 cypast61
    USING (PERSON_ID)
-- Diabetes indicators
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_DM_61 dm61
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_DM_62 dm62
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_DM_63 dm63
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_DM_64 dm64
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_DM_65 dm65
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_DM_66 dm66
    USING (PERSON_ID)
-- Hypertension indicators
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_HTN_61 htn61
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_HTN_62 htn62
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_HTN_63 htn63
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_HTN_65 htn65
    USING (PERSON_ID)
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_HTN_66 htn66
    USING (PERSON_ID);
