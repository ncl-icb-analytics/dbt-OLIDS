create or replace dynamic table DATA_LAB_NCL_TRAINING_TEMP.HEI_MIGRATION.DIM_PERSON_AGE(
    ...
    BIRTH_DATE_APPROX DATE, -- Approximate date of birth, calculated as the exact midpoint of the birth month/year
    DEATH_YEAR NUMBER, -- Year of death (NULL if alive)
    DEATH_MONTH NUMBER, -- Month of death (NULL if alive)
    DEATH_DATE_APPROX DATE, -- Approximate date of death, calculated as the exact midpoint of the death month/year (NULL if alive)
    IS_DECEASED BOOLEAN, -- Whether the person is recorded as deceased
    AGE NUMBER, -- Calculated age in full years (age at death for deceased, current age for living)
    AGE_MONTHS NUMBER, -- Calculated age in total months (age at death for deceased, current age for living)
    AGE_WEEKS_APPROX NUMBER, -- Calculated approximate age in total weeks (age at death for deceased, current age for living)
    AGE_DAYS_APPROX NUMBER, -- Calculated approximate age in total days (age at death for deceased, current age for living)
    AGE_BAND_5Y VARCHAR, -- 5-year age band (e.g., '0-4', '5-9')
    AGE_BAND_10Y VARCHAR, -- 10-year age band (e.g., '0-9', '10-19')
    AGE_BAND_NHS VARCHAR, -- Age band following NHS-style categories commonly used in healthcare reporting
...
COMMENT = 'Dimension table providing comprehensive age-related attributes for each person. Uses dim_person_birth_death as source for birth/death data, which calculates birth/death dates as the exact mathematical midpoint of each month for optimal statistical precision (e.g., 16th for 31-day months, 15th for February). Calculates age in various units (years, months, weeks, days) and derives multiple standard age bands, life stages, and UK school stages. For deceased persons, calculates age at death. For living persons, calculates current age.'
target_lag = '4 hours'
refresh_mode = AUTO
...
SELECT
    ...
    -- Derives age bands following actual NHS Digital categories used in Health Survey for England and General Practice Profiles
    CASE
        WHEN ac.AGE < 0 THEN 'Unknown'
