CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PERSON_DEMOGRAPHICS (
    -- Core Identifiers
...
FROM DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PERSON_AGE age

-- Join demographics
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PERSON_SEX sex
    ON age.PERSON_ID = sex.PERSON_ID

LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PERSON_ETHNICITY eth
    ON age.PERSON_ID = eth.PERSON_ID

LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PERSON_MAIN_LANGUAGE lang
    ON age.PERSON_ID = lang.PERSON_ID

-- Join practice information
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PERSON_CURRENT_PRACTICE prac
    ON age.PERSON_ID = prac.PERSON_ID

-- Join practice neighbourhood information
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PRACTICE_NEIGHBOURHOOD nbhd
    ON prac.PRACTICE_CODE = nbhd.PRACTICE_CODE

-- Join practice PCN and commissioning information
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PRACTICE_PCN pcn
