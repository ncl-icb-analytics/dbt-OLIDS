CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_LTC_LCS_CF_DM_64 (
    PERSON_ID VARCHAR, -- Unique identifier for the person
    SK_PATIENT_ID VARCHAR, -- Surrogate key for the patient
    AGE NUMBER, -- Age of the patient
    HAS_HIGH_BMI BOOLEAN, -- Flag indicating if person has high BMI based on ethnicity
...
COMMENT = 'Dimension table for LTC LCS case finding indicator DM_64: Patients who meet ALL of the following criteria:
1. High BMI based on ethnicity:
   - BMI ≥ 32.5 for BAME patients
   - BMI ≥ 35 for non-BAME patients
...
SELECT
    ...
    bp.AGE,
    CASE
        WHEN (bame.IS_BAME = TRUE AND bmi.LATEST_BMI_VALUE >= 32.5) OR
             (bame.IS_BAME IS NULL AND bmi.LATEST_BMI_VALUE >= 35) THEN TRUE
        ELSE FALSE
    END AS HAS_HIGH_BMI,
...
LEFT JOIN RecentHBA1C hba1c
    USING (PERSON_ID)
WHERE ((bame.IS_BAME = TRUE AND bmi.LATEST_BMI_VALUE >= 32.5) OR
       (bame.IS_BAME IS NULL AND bmi.LATEST_BMI_VALUE >= 35))
