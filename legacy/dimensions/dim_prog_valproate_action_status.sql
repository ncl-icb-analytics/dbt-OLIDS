...
CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_VALPROATE_ACTION_STATUS (
...
WITH BasePatientCohort AS (
    ...
    SELECT
        ...
        vscope.AGE
    FROM
        DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_VALPROATE_DB_SCOPE vscope
),
PPPStatusData AS (
    ...
    FROM DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_VALPROATE_PPP_STATUS
),
ARAFStatusData AS (
    ...
    FROM
        DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_VALPROATE_ARAF
),
PregnancyRiskData AS (
    ...
    FROM
        DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_PERM_ABSENCE_PREG_RISK
),
LearningDisabilityData AS (
    ...
    FROM
        DATA_LAB_OLIDS_UAT.HEI_MIGRATION.FCT_PERSON_DX_LD dxld
    WHERE
...
PregnancyData AS (
    ...
    FROM
        DATA_LAB_OLIDS_UAT.HEI_MIGRATION.FCT_PERSON_PREGNANT preg
    WHERE
...
NeurologyData AS (
    ...
    FROM
        DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_VALPROATE_NEUROLOGY
),
PsychiatryData AS (
    ...
    FROM
        DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PROG_VALPROATE_PSYCHIATRY
),
ConsolidatedData AS (
    SELECT
        bpc.PERSON_ID,
