CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_NCL_TRAINING_TEMP.HEI_MIGRATION.INTERMEDIATE_ASTHMA_ORDERS_12M (
    ...
    RECENT_ORDER_COUNT NUMBER -- Count of orders in the last 12 months
)
COMMENT = 'Intermediate table containing asthma medication orders from the last 12 months. Filters based on ASTTRT_COD cluster ID from MAPPED_CONCEPTS.'
TARGET_LAG = '4 hours'
REFRESH_MODE = AUTO
INITIALIZE = ON_CREATE
WAREHOUSE = NCL_ANALYTICS_XS
AS
WITH BaseAsthmaOrders AS (
    -- Fetches medication orders for asthma medications (ASTTRT_COD)
...
OrderCounts AS (
    -- Counts the number of orders per person in the last 12 months
    SELECT
        PERSON_ID,
        COUNT(*) as RECENT_ORDER_COUNT
    FROM BaseAsthmaOrders
    GROUP BY PERSON_ID
)
