CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_NCL_TRAINING_TEMP.HEI_MIGRATION.INTERMEDIATE_ALLERGY_ORDERS_ALL (
...
COMMENT = 'Intermediate table containing all allergy medication orders (BNF section 3.4). Includes orders for antihistamines such as Fexofenadine, Cetirizine, Loratadine, and other allergy medications.'
TARGET_LAG = '4 hours'
REFRESH_MODE = AUTO
INITIALIZE = ON_CREATE
WAREHOUSE = NCL_ANALYTICS_XS
AS
WITH BaseAllergyOrders AS (
    -- Get all medication orders for allergy medications
    SELECT
    ...
    WHERE bnf.BNF_CODE LIKE '0304%' -- Antihistamines and allergy medications
),
OrderCounts AS (
...
FROM BaseAllergyOrders bao
LEFT JOIN OrderCounts oc
    ON bao.PERSON_ID = oc.PERSON_ID;
