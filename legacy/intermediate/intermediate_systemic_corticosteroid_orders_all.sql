CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_NCL_TRAINING_TEMP.HEI_MIGRATION.INTERMEDIATE_SYSTEMIC_CORTICOSTEROID_ORDERS_ALL (
    ...
    CORTICOSTEROID_TYPE VARCHAR, -- Type of systemic corticosteroid (REPLACEMENT, GLUCOCORTICOID)
    RECENT_ORDER_COUNT NUMBER -- Count of orders in the last 6 months
)
COMMENT = 'Intermediate table containing all systemic (endocrine) corticosteroid medication orders (BNF section 6.3). Includes:
- Replacement therapy (6.3.1)
- Glucocorticoid therapy (6.3.2)'
TARGET_LAG = '4 hours'
REFRESH_MODE = AUTO
...
WITH BaseSystemicCorticosteroidOrders AS (
    ...
    WHERE bnf.BNF_CODE LIKE '0603%' -- All systemic corticosteroids
),
OrderCounts AS (
    ...
    FROM BaseSystemicCorticosteroidOrders
