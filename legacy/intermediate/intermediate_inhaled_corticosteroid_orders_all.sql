CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_NCL_TRAINING_TEMP.HEI_MIGRATION.INTERMEDIATE_INHALED_CORTICOSTEROID_ORDERS_ALL (
    ...
    CORTICOSTEROID_TYPE VARCHAR, -- Type of inhaled corticosteroid (SINGLE_AGENT, COMBINATION)
    RECENT_ORDER_COUNT NUMBER -- Count of orders in the last 6 months
)
COMMENT = 'Intermediate table containing all inhaled corticosteroid medication orders (BNF section 3.2). Includes:
- Single agent corticosteroids
- Combination products (with long-acting beta agonists and/or long-acting muscarinic antagonists)'
TARGET_LAG = '4 hours'
REFRESH_MODE = AUTO
...
WITH BaseInhaledCorticosteroidOrders AS (
    ...
    WHERE bnf.BNF_CODE LIKE '0302%' -- All inhaled corticosteroids
),
OrderCounts AS (
    ...
    FROM BaseInhaledCorticosteroidOrders
