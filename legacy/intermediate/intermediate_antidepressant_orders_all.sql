CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_NCL_TRAINING_TEMP.HEI_MIGRATION.INTERMEDIATE_ANTIDEPRESSANT_ORDERS_ALL (
    ...
    ANTIDEPRESSANT_CLASS VARCHAR, -- Class of antidepressant (SSRI, SNRI, TCA, MAOI, OTHER)
    RECENT_ORDER_COUNT NUMBER -- Count of orders in the last 6 months
)
COMMENT = 'Intermediate table containing all antidepressant medication orders (BNF section 4.3). Includes:
- Selective serotonin re-uptake inhibitors (SSRIs) (4.3.3)
- Serotonin and noradrenaline re-uptake inhibitors (SNRIs) (4.3.4)
- Tricyclic and related antidepressant drugs (4.3.1)
- Monoamine-oxidase inhibitors (MAOIs) (4.3.2)
- Other antidepressant drugs (4.3.5)'
TARGET_LAG = '4 hours'
REFRESH_MODE = AUTO
...
WITH BaseAntidepressantOrders AS (
    ...
    SELECT
        ...
        CASE
            ...
            WHEN bnf.BNF_CODE LIKE '040302%' THEN 'MAOI'
            WHEN bnf.BNF_CODE LIKE '040305%' THEN 'OTHER'
            ELSE 'UNKNOWN'
    ...
    WHERE bnf.BNF_CODE LIKE '0403%' -- All antidepressants
),
OrderCounts AS (
