CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_LTC_LCS_CF_CVD_BASE (
    PERSON_ID VARCHAR,
...
COMMENT = 'Base population for CVD case finding indicators. Includes patients aged 40-83 who are not on statins, have no statin allergies/contraindications, and no recent statin decisions.'
TARGET_LAG = '4 hours'
REFRESH_MODE = AUTO
...
WITH BasePopulation AS (
    -- Get base population of patients aged 40-83
    SELECT DISTINCT
        bp.PERSON_ID,
        bp.SK_PATIENT_ID,
        age.AGE
    ...
    JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.DIM_PERSON_AGE age
        USING (PERSON_ID)
    WHERE age.AGE BETWEEN 40 AND 83
),
