CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_OLIDS_UAT.HEI_MIGRATION.FCT_PERSON_DIABETES_8_CARE_PROCESSES(
...
FROM DATA_LAB_OLIDS_UAT.HEI_MIGRATION.FCT_PERSON_DX_DIABETES dr
CROSS JOIN TwelveMonthsAgo t
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_HBA1C_LATEST hba
...
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_TOTAL_CHOLESTEROL_LATEST chol
    ON dr.PERSON_ID = chol.PERSON_ID
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_SERUM_CREATININE_LATEST cre
    ON dr.PERSON_ID = cre.PERSON_ID
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_URINE_ACR_LATEST acr
    ON dr.PERSON_ID = acr.PERSON_ID
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_FOOT_CHECK_LATEST fc
    ON dr.PERSON_ID = fc.PERSON_ID
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_BMI_LATEST bmi
    ON dr.PERSON_ID = bmi.PERSON_ID
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_SMOKING_LATEST smok
