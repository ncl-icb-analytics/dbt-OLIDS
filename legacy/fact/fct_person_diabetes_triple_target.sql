CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_OLIDS_UAT.HEI_MIGRATION.FCT_PERSON_DIABETES_TRIPLE_TARGET(
...
SELECT
    ...
    (
        hba1c.RESULT_VALUE IS NOT NULL AND (
            (hba1c.IS_IFCC AND hba1c.RESULT_VALUE < 58)
            OR (hba1c.IS_DCCT AND hba1c.RESULT_VALUE < 7.5)
        )
    ...
    (
        ...
        AND (bp.SYSTOLIC_VALUE IS NOT NULL AND bp.SYSTOLIC_VALUE < 140 AND bp.DIASTOLIC_VALUE IS NOT NULL AND bp.DIASTOLIC_VALUE < 80)
    ...
    (
        (hba1c.CLINICAL_EFFECTIVE_DATE IS NOT NULL AND DATEDIFF(month, hba1c.CLINICAL_EFFECTIVE_DATE, CURRENT_DATE()) <= 12)
        AND NOT (
            hba1c.RESULT_VALUE IS NOT NULL AND (
                (hba1c.IS_IFCC AND hba1c.RESULT_VALUE < 58)
                OR (hba1c.IS_DCCT AND hba1c.RESULT_VALUE < 7.5)
...
LEFT JOIN DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_HBA1C_LATEST hba1c
