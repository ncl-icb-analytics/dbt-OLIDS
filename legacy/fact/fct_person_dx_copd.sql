CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_OLIDS_UAT.HEI_MIGRATION.FCT_PERSON_DX_COPD (
...
WITH BaseDiagnoses AS (
    ...
    FROM DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_COPD_DIAGNOSES d
...
LatestSpirometry AS (
    ...
    FROM DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_SPIROMETRY
    GROUP BY PERSON_ID
),
UnableSpirometry AS (
    ...
    FROM DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_COPD_UNABLE_SPIROMETRY
    GROUP BY PERSON_ID
),
PersonLevelCodingAggregation AS (
    ...
    FROM DATA_LAB_OLIDS_UAT.HEI_MIGRATION.INTERMEDIATE_COPD_DIAGNOSES
