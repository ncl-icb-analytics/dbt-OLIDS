CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_NCL_TRAINING_TEMP.HEI_MIGRATION.FCT_PERSON_DX_ASTHMA (
    PERSON_ID VARCHAR, -- Unique identifier for a person
    SK_PATIENT_ID VARCHAR, -- Surrogate key for the patient
    AGE NUMBER, -- Age of the person (>= 6 for this table)
...
COMMENT = 'Fact table identifying individuals aged 6 and over currently on the Asthma register. Requires both an active asthma diagnosis (latest AST_COD > latest ASTRES_COD) and a recent asthma medication order (within last 12 months).'
TARGET_LAG = '4 hours'
REFRESH_MODE = AUTO
...
FilteredByAge AS (
    -- Filters the base asthma-related observations to include only individuals aged 6 or older.
    SELECT
        boc.*,
        age.AGE
...
PersonLevelAsthmaCodingAggregation AS (
    -- Aggregates asthma diagnosis and resolution code information for each person aged 6+.
...
LatestMedicationDetails AS (
    ...
    FROM DATA_LAB_NCL_TRAINING_TEMP.HEI_MIGRATION.INTERMEDIATE_ASTHMA_ORDERS_12M
    GROUP BY PERSON_ID
)
