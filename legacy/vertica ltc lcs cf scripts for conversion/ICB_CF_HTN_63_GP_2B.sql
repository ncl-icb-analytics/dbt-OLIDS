...
INNER JOIN JOINED_LTC_LOOKUP AS L ON A.ORIG_CODIFIED_VALUE_CODE = L.SNOMED_CODE 

WHERE CLUSTER_ID = 'HTN_BSA_COD' )

-- Select latest reading for systolic and diastolic
,RULE_4_TABLE AS (
SELECT EMPI_ID, CLUSTER_ID, SOURCE_DESCRIPTION, RESULT_CODE, NORM_NUMERIC_VALUE

FROM (
SELECT DISTINCT P.EMPI_ID, SOURCE_DESCRIPTION, L.CLUSTER_ID, R.RESULT_CODE, NORM_NUMERIC_VALUE, SERVICE_DATE
,ROW_NUMBER() OVER (PARTITION BY P.EMPI_ID, R.SOURCE_DESCRIPTION, L.CLUSTER_ID ORDER BY R.SERVICE_DATE DESC, NORM_NUMERIC_VALUE DESC) AS ROW_NUMBER

FROM BSA_Population AS P
INNER JOIN PH_F_RESULT AS R ON P.EMPI_ID = R.EMPI_ID
INNER JOIN JOINED_LTC_LOOKUP AS L ON L.SNOMED_CODE = R.RESULT_CODE

WHERE L.CLUSTER_ID IN ('HTN_SYSTOLIC_1', 'HTN_DIASTOLIC_1')
...
FROM (
SELECT DISTINCT EMPI_ID, SERVICE_DATE, SOURCE_DESCRIPTION, NORM_NUMERIC_VALUE
,ROW_NUMBER() OVER (PARTITION BY EMPI_ID, SOURCE_DESCRIPTION ORDER BY SERVICE_DATE DESC) AS ROW_NUMBER

FROM PH_F_RESULT AS R
INNER JOIN JOINED_LTC_LOOKUP AS L ON R.RESULT_CODE = L.SNOMED_CODE

WHERE 
L.CLUSTER_ID = 'HTN_EGFR' ) AS GFR
...
FROM (
SELECT DISTINCT EMPI_ID, SERVICE_DATE, SOURCE_DESCRIPTION, NORM_NUMERIC_VALUE
,ROW_NUMBER() OVER (PARTITION BY EMPI_ID, SOURCE_DESCRIPTION ORDER BY SERVICE_DATE DESC, NORM_NUMERIC_VALUE DESC) AS ROW_NUMBER

FROM PH_F_RESULT AS R
INNER JOIN JOINED_LTC_LOOKUP AS L ON R.RESULT_CODE = L.SNOMED_CODE

WHERE 
L.CLUSTER_ID = 'HTN_BMI' ) AS BMI
...
WHERE SOURCE_DESCRIPTION = 'EMIS GP' AND 
R4.CLUSTER_ID = 'HTN_SYSTOLIC_1' AND 
L.CLUSTER_ID = 'HTN_SYSTOLIC_2' AND
NORM_NUMERIC_VALUE >= 140
...
WHERE SOURCE_DESCRIPTION = 'EMIS GP' AND 
R4.CLUSTER_ID = 'HTN_DIASTOLIC_1' AND 
L.CLUSTER_ID = 'HTN_DIASTOLIC_2' AND
NORM_NUMERIC_VALUE >= 90
