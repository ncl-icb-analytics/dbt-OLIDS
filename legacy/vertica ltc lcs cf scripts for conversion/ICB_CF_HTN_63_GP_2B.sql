...
-- Creating a population from base that excludes the highest priority patients from
-- ICB_CF_HTN_61_GP_1 and ICB_CF_HTN_62_GP_2A
WITH Population AS (
...
BSA_Population AS (
    ...
    SELECT DISTINCT P.EMPI_ID

    FROM Population AS P
    INNER JOIN PH_F_QUESTIONNAIRE_ANSWER AS A ON P.EMPI_ID = A.EMPI_ID
    INNER JOIN
    ...
    FROM (
        SELECT DISTINCT
            ...
            ROW_NUMBER()
                OVER (
                    PARTITION BY EMPI_ID, SOURCE_DESCRIPTION
                    ORDER BY SERVICE_DATE DESC, NORM_NUMERIC_VALUE DESC
                )
                AS ROW_NUMBER
        ...
        WHERE
            L.CLUSTER_ID = 'HTN_BMI'
    ) AS BMI

    WHERE ROW_NUMBER = '1' AND NORM_NUMERIC_VALUE > 35
