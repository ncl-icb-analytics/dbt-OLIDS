...
EMIS AS (
    WITH LATEST_BMI AS (
        ...
        FROM (
            SELECT
                ...
                ROW_NUMBER()
                    OVER (
                        PARTITION BY BASE_POPULATION.EMPI_ID
            ...
            LEFT OUTER JOIN
                NCL_CODES
                ON BASE_POPULATION.EMPI_ID = NCL_CODES.EMPI_ID
            WHERE
    ...
    WHERE EXC.EMPI_ID IS NULL AND (
        (LATEST_BMI.IS_BAME = 1 AND CAST(LATEST_BMI.VALUE AS FLOAT) >= 32.5)
        OR (LATEST_BMI.IS_BAME = 0 AND CAST(LATEST_BMI.VALUE AS FLOAT) >= 35)
    )
),
...
OTHER AS (
    ...
    WHERE EXC.EMPI_ID IS NULL AND (
        (
            ...
            AND CAST(LATEST_BMI_OTHER.VALUE AS FLOAT) >= 32.5
        )
        OR (
            LATEST_BMI_OTHER.IS_BAME = 0
            AND CAST(LATEST_BMI_OTHER.VALUE AS FLOAT) >= 35
        )
    )
),
