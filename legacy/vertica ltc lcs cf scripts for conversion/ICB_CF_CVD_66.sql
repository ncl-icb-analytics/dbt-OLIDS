...
LEFT JOIN (SELECT EMPI_ID FROM CARDIAC_RISK_SCORES_EMIS) AS EMIS ON subquery.EMPI_ID = EMIS.EMPI_ID
LEFT JOIN (SELECT EMPI_ID FROM CARDIAC_RISK_SCORES_OTHER) AS OTHER ON subquery.EMPI_ID = OTHER.EMPI_ID
INNER JOIN ICB_CF_CVD_66_BASE AS BASE ON BASE.EMPI_ID = subquery.EMPI_ID
LEFT JOIN LTC_LCS_BASE AS LCS ON LCS.EMPI_ID = subquery.EMPI_ID
