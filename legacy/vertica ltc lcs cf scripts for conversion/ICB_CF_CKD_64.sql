...
WITH HASHED_POP AS (
    ...
    FROM (
        SELECT
            EMPI_ID,
            HASH(EMPI_ID) // 2e18 AS hash_value
...
TEMP_PH_F_RESULT AS (
    ...
    INNER JOIN HASHED_POP ON C.EMPI_ID = HASHED_POP.EMPI_ID
    WHERE
        C.RESULT_CODE = '1023491000000104'
...
TEMP_PH_F_CONDITION AS (
    SELECT
        C.EMPI_ID,
        SOURCE_DESCRIPTION,
        EFFECTIVE_DT_TM AS DATE,
        T.CLUSTER_ID

    FROM PH_F_CONDITION AS C
    INNER JOIN HASHED_POP ON C.EMPI_ID = HASHED_POP.EMPI_ID
    INNER JOIN JOINED_LTC_LOOKUP AS T ON C.CONDITION_CODE = T.SNOMED_CODE
    ...
    SELECT
        C.EMPI_ID,
        SOURCE_DESCRIPTION,
        C.SERVICE_START_DT_TM AS DATE,
        T.CLUSTER_ID
    FROM PH_F_PROCEDURE AS C
    INNER JOIN HASHED_POP ON C.EMPI_ID = HASHED_POP.EMPI_ID
    INNER JOIN JOINED_LTC_LOOKUP AS T ON C.PROCEDURE_CODE = T.SNOMED_CODE
    WHERE T.CLUSTER_ID IN ('CKD_BPH_GOUT', 'CKD_AKI', 'Hematuria', 'Urine_Blood_Neg', 'Proteinuria')--remove P once clusters updated
    ...
    SELECT
        C.EMPI_ID,
        SOURCE_DESCRIPTION,
        C.SERVICE_DATE AS DATE,
        T.CLUSTER_ID
    FROM PH_F_RESULT AS C
    INNER JOIN HASHED_POP ON C.EMPI_ID = HASHED_POP.EMPI_ID
    INNER JOIN JOINED_LTC_LOOKUP AS T ON C.RESULT_CODE = T.SNOMED_CODE
    WHERE T.CLUSTER_ID IN ('CKD_BPH_GOUT', 'CKD_AKI', 'Hematuria', 'Urine_Blood_Neg', 'Proteinuria')--remove P once clusters updated
...
RELEVANT_DRUG_CODES AS (
    SELECT M.DRUG_CODE
    FROM CERNER_MEDICATION_JOINED_TABLE AS M
    INNER JOIN HASHED_POP ON M.EMPI_ID = HASHED_POP.EMPI_ID
    LEFT JOIN JOINED_LTC_LOOKUP AS T ON M.DRUG_CODE = T.SNOMED_CODE
...
EMIS AS (
    ...
    FROM
        (
            ...
            INNER JOIN HASHED_POP ON B.EMPI_ID = HASHED_POP.EMPI_ID
            INNER JOIN TEMP_PH_F_CONDITION AS C ON B.EMPI_ID = C.EMPI_ID
            WHERE
                C.CLUSTER_ID IN ('CKD_AKI')
            ...
            INNER JOIN
                ...
                ON B.EMPI_ID = M.EMPI_ID
            WHERE
                M.DRUG_CODE IN (SELECT DRUG_CODE FROM RELEVANT_DRUG_CODES)
            ...
            SELECT * FROM (
                WITH MICRO_HAEM AS (
                    ...
                    FROM (
                        ...
                        INNER JOIN HASHED_POP ON B.EMPI_ID = HASHED_POP.EMPI_ID
                        INNER JOIN
                            TEMP_PH_F_CONDITION AS C
                            ON B.EMPI_ID = C.EMPI_ID
                        WHERE
                            C.CLUSTER_ID = 'Hematuria'
                            AND C.SOURCE_DESCRIPTION = 'EMIS GP'
                            AND B.EMIS = '1'
                    ) AS A
                    WHERE ROW_NUMBER = 1
                ...
                URINE_NEG AS (
                    ...
                    WHERE
                        C.CLUSTER_ID IN ('Urine_Blood_Neg', 'Proteinuria')--remove P once clusters updated
                        AND C.SOURCE_DESCRIPTION = 'EMIS GP'
    ...
    FROM
        (
            ...
            SELECT * FROM (
                ...
                WHERE
                    R.EMPI_ID IS NULL
                    OR (R.EMPI_ID IS NOT NULL AND H.DATE > R.DATE)
