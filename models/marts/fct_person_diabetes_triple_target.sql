...
SELECT
    ...
    (bp.systolic_value IS NOT NULL AND bp.diastolic_value IS NOT NULL 
     AND bp.systolic_value < 130 AND bp.diastolic_value < 80) AS bp_in_target_range,
    ...
    (
        ...
        AND (bp.systolic_value IS NOT NULL AND bp.diastolic_value IS NOT NULL 
    ...
    (hba.clinical_effective_date IS NOT NULL 
     ...
     AND NOT COALESCE(hba.meets_qof_target, FALSE)) AS hba1c_recent_but_out_of_range,
    
    (bp.clinical_effective_date IS NOT NULL 
     AND bp.clinical_effective_date >= t.twelve_months_ago
     AND NOT (bp.systolic_value IS NOT NULL AND bp.diastolic_value IS NOT NULL 
              AND bp.systolic_value < 130 AND bp.diastolic_value < 80)) AS bp_recent_but_out_of_range,
