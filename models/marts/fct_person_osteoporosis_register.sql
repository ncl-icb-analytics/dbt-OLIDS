...
WITH osteoporosis_diagnoses AS (
    SELECT
        ...
        is_dxa_scan AS has_dxa_scan,
        is_dxa_t_score AS has_dxa_t_score,
...
fragility_fractures AS (
    SELECT
        person_id,
        is_fragility_fracture AS has_fragility_fracture,
...
register_logic AS (
    SELECT
        ...
        CASE
            WHEN diag.has_dxa_scan = TRUE THEN TRUE
            WHEN diag.has_dxa_t_score = TRUE AND diag.latest_dxa_t_score <= -2.5 THEN TRUE
            ELSE FALSE
        END AS has_valid_dxa_confirmation,
        
        -- Additional component flags for transparency
        COALESCE(diag.has_dxa_scan, FALSE) AS has_dxa_scan,
        COALESCE(diag.has_dxa_t_score, FALSE) AS has_dxa_t_score,
        ...
        CASE
            WHEN age.age BETWEEN 50 AND 74
                ...
                AND (
                    diag.has_dxa_scan = TRUE OR 
                    (diag.has_dxa_t_score = TRUE AND diag.latest_dxa_t_score <= -2.5)
                )
            THEN TRUE
            ELSE FALSE
        ...
        -- Clinical dates - DXA
        diag.earliest_dxa_date,
        diag.latest_dxa_date,
        diag.earliest_dxa_t_score_date,
        diag.latest_dxa_t_score_date,
        diag.latest_dxa_t_score,
        ...
        -- Traceability - DXA
        diag.all_dxa_concept_codes,
        diag.all_dxa_concept_displays,
