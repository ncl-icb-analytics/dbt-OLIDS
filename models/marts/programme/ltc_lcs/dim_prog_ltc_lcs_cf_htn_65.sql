...
higher_priority_patients AS (
    ...
    SELECT DISTINCT person_id
    FROM (
        SELECT person_id FROM {{ ref('dim_prog_ltc_lcs_cf_htn_61') }}
        UNION
        SELECT person_id FROM {{ ref('dim_prog_ltc_lcs_cf_htn_62') }}
        UNION
        SELECT person_id FROM {{ ref('dim_prog_ltc_lcs_cf_htn_63') }}
    )
),
...
patients_with_risk_factors AS (
    ...
    JOIN risk_factor_patients rf USING (person_id)
    WHERE NOT EXISTS (
...
eligible_patients AS (
    ...
    FROM latest_bp bp
    JOIN patients_with_risk_factors prf USING (person_id)
    WHERE (
