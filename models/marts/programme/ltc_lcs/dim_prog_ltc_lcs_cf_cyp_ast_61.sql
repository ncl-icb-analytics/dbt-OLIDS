...
WITH cyp_base_population AS (
    ...
    JOIN {{ ref('dim_person_age') }} age USING (person_id)
...
asthma_symptoms_obs AS (
    ...
    FROM {{ ref('int_ltc_lcs_cyp_asthma_observations') }}
    WHERE cluster_id IN ('SUSPECTED_ASTHMA', 'VIRAL_WHEEZE')
...
asthma_symptoms AS (
    ...
    FROM cyp_base_population cyp
    LEFT JOIN asthma_medications meds USING (person_id)
    LEFT JOIN asthma_symptoms_obs obs USING (person_id)
...
eligible_patients AS (
    ...
    FROM cyp_base_population cyp
    JOIN asthma_symptoms symptoms USING (person_id)
    WHERE NOT EXISTS (
        SELECT 1 FROM asthma_diagnosis ad 
