...
eligible_patients AS (
    ...
    JOIN {{ ref('int_ltc_lcs_cf_base_population') }} base USING (person_id)
    WHERE NOT EXISTS (
        SELECT 1 FROM {{ ref('dim_prog_ltc_lcs_cf_htn_61') }} htn61 
        WHERE htn61.person_id = bp.person_id
    )
    AND (
        (bp.is_clinic_bp AND (bp.systolic_value >= 160 OR bp.diastolic_value >= 100))
        OR 
        (bp.is_home_bp AND (bp.systolic_value >= 150 OR bp.diastolic_value >= 95))
    )
)
