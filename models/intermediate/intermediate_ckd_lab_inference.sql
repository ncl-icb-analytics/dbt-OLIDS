...
COMMENT = 'Intermediate table inferring Chronic Kidney Disease (CKD) status and stage based on lab results (eGFR and ACR). It determines the latest G and A stages, calculates a combined inferred stage, and checks for confirmation of CKD based on persistent abnormal results (eGFR < 60 or ACR >= 3) over a >90 day period within approximately 2 years. Note: CKD Confirmation through labs does not mean the person should be clinically diagnosed with CKD, only that the lab results are persistent and consistent with a CKD diagnosis; they are strong candidates for case finding if not already diagnosed or if not coded with the correct stage of CKD. Use FCT_PERSON_CURRENT_DX_CKD to determine if the person has a clinical diagnosis of CKD.'
TARGET_LAG = '4 hours'
REFRESH_MODE = AUTO
