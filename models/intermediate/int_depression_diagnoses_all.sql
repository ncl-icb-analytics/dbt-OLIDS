...
WITH base_observations AS (
    ...
    SELECT
        ...
        CASE WHEN obs.source_cluster_id = 'DEPR_COD' THEN TRUE ELSE FALSE END AS is_depression_diagnosis_code,
        CASE WHEN obs.source_cluster_id = 'DEPRES_COD' THEN TRUE ELSE FALSE END AS is_depression_resolved_code,
        CASE WHEN obs.source_cluster_id = 'DEPRVW_COD' THEN TRUE ELSE FALSE END AS is_depression_review_code,
        CASE WHEN obs.source_cluster_id = 'DEPRINVITE_COD' THEN TRUE ELSE FALSE END AS is_depression_invite_code,
...
person_aggregates AS (
    ...
    SELECT
        ...
        ARRAY_AGG(DISTINCT CASE WHEN is_depression_diagnosis_code THEN concept_code END) 
            FILTER (WHERE is_depression_diagnosis_code) AS all_depression_concept_codes,
        ARRAY_AGG(DISTINCT CASE WHEN is_depression_diagnosis_code THEN concept_display END) 
            FILTER (WHERE is_depression_diagnosis_code) AS all_depression_concept_displays,
        ARRAY_AGG(DISTINCT CASE WHEN is_depression_resolved_code THEN concept_code END) 
            FILTER (WHERE is_depression_resolved_code) AS all_resolved_concept_codes,
        ARRAY_AGG(DISTINCT CASE WHEN is_depression_review_code THEN concept_code END) 
            FILTER (WHERE is_depression_review_code) AS all_review_concept_codes,
        ARRAY_AGG(DISTINCT CASE WHEN is_depression_invite_code THEN concept_code END) 
            FILTER (WHERE is_depression_invite_code) AS all_invite_concept_codes,
        ARRAY_AGG(DISTINCT CASE WHEN is_depression_pca_decline_code THEN concept_code END) 
            FILTER (WHERE is_depression_pca_decline_code) AS all_pca_decline_concept_codes,
        ARRAY_AGG(DISTINCT CASE WHEN is_depression_pca_unsuitable_code THEN concept_code END) 
            FILTER (WHERE is_depression_pca_unsuitable_code) AS all_pca_unsuitable_concept_codes
