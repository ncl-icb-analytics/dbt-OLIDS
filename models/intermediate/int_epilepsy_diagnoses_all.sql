...
WITH base_observations AS (
    ...
    SELECT
        ...
        CASE WHEN obs.source_cluster_id = 'EPIL_COD' THEN TRUE ELSE FALSE END AS is_epilepsy_diagnosis_code,
        CASE WHEN obs.source_cluster_id = 'EPILRES_COD' THEN TRUE ELSE FALSE END AS is_epilepsy_resolved_code
...
person_aggregates AS (
    ...
    SELECT
        ...
        ARRAY_AGG(DISTINCT CASE WHEN is_epilepsy_diagnosis_code THEN concept_code END) 
            FILTER (WHERE is_epilepsy_diagnosis_code) AS all_epilepsy_concept_codes,
        ARRAY_AGG(DISTINCT CASE WHEN is_epilepsy_diagnosis_code THEN concept_display END) 
            FILTER (WHERE is_epilepsy_diagnosis_code) AS all_epilepsy_concept_displays,
        ARRAY_AGG(DISTINCT CASE WHEN is_epilepsy_resolved_code THEN concept_code END) 
            FILTER (WHERE is_epilepsy_resolved_code) AS all_resolved_concept_codes,
        ARRAY_AGG(DISTINCT CASE WHEN is_epilepsy_resolved_code THEN concept_display END) 
            FILTER (WHERE is_epilepsy_resolved_code) AS all_resolved_concept_displays
