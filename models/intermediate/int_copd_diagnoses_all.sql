...
WITH base_observations AS (
    ...
    SELECT
        ...
        CASE WHEN obs.source_cluster_id = 'COPD_COD' THEN TRUE ELSE FALSE END AS is_copd_diagnosis_code,
        CASE WHEN obs.source_cluster_id = 'COPDRES_COD' THEN TRUE ELSE FALSE END AS is_copd_resolved_code
...
person_aggregates AS (
    ...
    SELECT
        ...
        ARRAY_AGG(DISTINCT CASE WHEN is_copd_diagnosis_code THEN concept_code END) 
            FILTER (WHERE is_copd_diagnosis_code) AS all_copd_concept_codes,
        ARRAY_AGG(DISTINCT CASE WHEN is_copd_diagnosis_code THEN concept_display END) 
            FILTER (WHERE is_copd_diagnosis_code) AS all_copd_concept_displays,
        ARRAY_AGG(DISTINCT CASE WHEN is_copd_resolved_code THEN concept_code END) 
            FILTER (WHERE is_copd_resolved_code) AS all_resolved_concept_codes,
        ARRAY_AGG(DISTINCT CASE WHEN is_copd_resolved_code THEN concept_display END) 
            FILTER (WHERE is_copd_resolved_code) AS all_resolved_concept_displays
