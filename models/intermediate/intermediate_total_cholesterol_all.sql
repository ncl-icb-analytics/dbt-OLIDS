CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_NCL_TRAINING_TEMP.HEI_MIGRATION.INTERMEDIATE_TOTAL_CHOLESTEROL_ALL(
    ...
    RESULT_VALUE NUMBER, -- The numeric result value of the cholesterol test (mmol/L)
...
COMMENT = 'Intermediate table containing all recorded total cholesterol results for all persons. Filters based on CHOL2_COD cluster. Excludes records with NULL result values.'
AS
SELECT DISTINCT
