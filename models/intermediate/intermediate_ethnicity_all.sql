...
CREATE OR REPLACE DYNAMIC TABLE DATA_LAB_NCL_TRAINING_TEMP.HEI_MIGRATION.INTERMEDIATE_ETHNICITY_ALL (
    ...
    CONCEPT_CODE VARCHAR, -- The specific concept code associated with the ethnicity observation
...
COMMENT = 'Intermediate table containing all recorded ethnicity observations for all persons. Filters based on the ETHNICITY_COD cluster ID from MAPPED_CONCEPTS.'
AS
-- Selects distinct ethnicity observation records.
...
-- Filter for observations belonging to the ethnicity code cluster.
WHERE C.CLUSTER_ID = 'ETHNICITY_COD';
