...
EMIS AS (
    WITH Latest_BMI AS (
        ...
        FROM (
            SELECT 
                ...
                ROW_NUMBER() OVER (PARTITION BY BASE_POPULATION_EMIS.EMPI_ID ORDER BY NCL_CODES.DATETIME DESC, NCL_CODES.VALUE DESC) AS Row_Num,
                NCL_CODES.VALUE
            FROM BASE_POPULATION_EMIS
            LEFT OUTER JOIN NCL_CODES ON BASE_POPULATION_EMIS.EMPI_ID = NCL_CODES.EMPI_ID
            WHERE 
    ...
    WHERE Exc.EMPI_ID IS NULL  AND (
            (Latest_BMI.Is_BAME = 1 AND CAST(Latest_BMI.VALUE AS FLOAT) BETWEEN 27.5 AND 32.5) OR
            (Latest_BMI.Is_BAME = 0 AND CAST(Latest_BMI.VALUE AS FLOAT) BETWEEN 30 AND 35)
    )
),
-- OTHER CTE
OTHER AS (
    WITH Latest_BMI_OTHER AS (
        ...
        FROM (
            SELECT 
                ...
                ROW_NUMBER() OVER (PARTITION BY BASE_POPULATION_ALL.EMPI_ID ORDER BY NCL_CODES.DATETIME DESC, NCL_CODES.VALUE DESC) AS Row_Num,
                NCL_CODES.VALUE
            FROM BASE_POPULATION_ALL
            LEFT OUTER JOIN NCL_CODES ON BASE_POPULATION_ALL.EMPI_ID = NCL_CODES.EMPI_ID
            WHERE 
                CODE IN (SELECT SNOMED_CODE FROM JOINED_LTC_LOOKUP WHERE CLUSTER_ID = 'BMI_CODES') AND
                NCL_CODES.VALUE IS NOT NULL
        ) AS A
    ...
    WHERE Exc.EMPI_ID IS NULL  AND (
            (Latest_BMI_OTHER.Is_BAME = 1 AND CAST(Latest_BMI_OTHER.VALUE AS FLOAT) BETWEEN 27.5 AND 32.5) OR
            (Latest_BMI_OTHER.Is_BAME = 0 AND CAST(Latest_BMI_OTHER.VALUE AS FLOAT) BETWEEN 30 AND 35)
    )
)
