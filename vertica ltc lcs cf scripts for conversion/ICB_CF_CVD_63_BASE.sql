...
WITH OTHER AS (
    SELECT 
        DISTINCT CARDIAC_RISK_SCORES.EMPI_ID,
        'Other' AS SOURCE
    FROM (
        SELECT 
            EMPI_ID,
    ...
    INNER JOIN LTC_LCS_BASE ON CARDIAC_RISK_SCORES.EMPI_ID = LTC_LCS_BASE.EMPI_ID
    WHERE rn = 1
    AND NORM_NUMERIC_VALUE >= 10
    AND AGE BETWEEN 40 AND 83
    EXCEPT 
    SELECT EMPI_ID, 'Other' FROM ICS_LTC_01
    EXCEPT
    SELECT EMPI_ID, 'Other' FROM POPHEALTH_QOF_LTCS_LIST WHERE LTC_NAME IN ('Diabetes', 'Palliative Care')
    EXCEPT
    SELECT EMPI_ID, 'Other' FROM NCL_CODES WHERE CODE IN (
...
EMIS AS (
    ...
    AND AGE BETWEEN 40 AND 83
    EXCEPT 
    SELECT EMPI_ID, 'EMIS' FROM ICS_LTC_01
    EXCEPT
    SELECT EMPI_ID, 'EMIS' FROM POPHEALTH_QOF_LTCS_LIST WHERE LTC_NAME IN ('Diabetes', 'Palliative Care')
    EXCEPT
    SELECT EMPI_ID, 'EMIS' FROM NCL_CODES WHERE CODE IN (
        SELECT SNOMED_CODE FROM JOINED_LTC_LOOKUP
        WHERE CLUSTER_ID IN ('STATIN_ALLERGY_ADVERSE_REACTION', 'STATIN_NOT_INDICATED')
    )
)
...
-- Main Query
SELECT DISTINCT ALL_IDS.EMPI_ID,
       COALESCE(
