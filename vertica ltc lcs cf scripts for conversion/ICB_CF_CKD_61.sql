...
PATIENT_LIST AS (
SELECT DISTINCT C.EMPI_ID, SOURCE_DESCRIPTION, SERVICE_DATE, NORM_NUMERIC_VALUE
...
-- FINAL TABLE 
SELECT DISTINCT EMPI_ID, SOURCE_DESCRIPTION FROM RECENT2 
...
INNER JOIN PH_F_RESULT AS C ON B.EMPI_ID = C.EMPI_ID
JOIN JOINED_LTC_LOOKUP t ON t.SNOMED_CODE = c.RESULT_CODE
AND (t.cluster_id IN ('EGFR_COD') OR c.result_code = '77147-7')
AND c.NORM_NUMERIC_VALUE >0
),
