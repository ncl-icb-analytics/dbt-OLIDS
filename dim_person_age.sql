...
SELECT
    ...
    -- NHS Digital standard age bands
    CASE
        WHEN ac.AGE < 0 THEN 'Unknown'
    ...
    -- UK school stage categorisation - Using BIRTH_DATE from ac for accuracy
    CASE
        WHEN ac.AGE < 0 THEN 'Unknown'
    ...
    -- Broader education level category - Using updated school stage logic
     CASE
        WHEN ac.AGE < 0 THEN 'Unknown'
        WHEN ac.AGE < 3 THEN 'Pre-school'
        WHEN ac.AGE = 3 OR (ac.AGE = 4 AND DATEDIFF(month, ac.BIRTH_DATE, DATE_FROM_PARTS(ac.ACADEMIC_YEAR_START, 9, 1)) < 48) THEN 'Nursery'
